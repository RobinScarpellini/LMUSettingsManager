...
class ConfigurationManager:
    ...
    def __init__(self, game_config_dir: Path):
        ...
        self.config_dir = game_config_dir

        # File paths
        self.active_json_file = self.config_dir / "settings.json"
        self.active_ini_file = self.config_dir / "Config_DX11.ini"

        # Metadata file for descriptions and timestamps
        self.metadata_file = self.config_dir / "lmu_config_metadata.json"
    ...
    def get_saved_configurations(self) -> List[str]:
        ...
        for json_file in self.config_dir.glob("conf_*_settings.json"):
            # Extract name from filename
            name = json_file.stem.replace("conf_", "").replace("_settings", "")

            # Check if corresponding INI file exists
            ini_file = self.config_dir / f"conf_{name}_Config_DX11.ini"
            if ini_file.exists():
    ...
    def save_configuration(
        ...
        try:
            ...
            ini_path = self.config_dir / ini_filename
    ...
    def load_configuration(self, name: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # Get file paths
            json_path = self.config_dir / f"conf_{name}_settings.json"
            ini_path = self.config_dir / f"conf_{name}_Config_DX11.ini"
    ...
    def get_configuration_info(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        if name in self.metadata["configurations"]:
            ...
            json_path = self.config_dir / info.get("json_file", "")
            ini_path = self.config_dir / info.get("ini_file", "")
